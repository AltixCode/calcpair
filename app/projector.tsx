...
export default function Projector() {
  ...
  const message = () => {
    switch (result.kind) {
      case 'reachable':
        return { text: t('needAverage', { average: result.average.toFixed(2) }), tone: 'default' as const };
    ...
    <View style={{ flex: 1, backgroundColor: colors.background, paddingTop: insets.top }}>
      ...
      >
        ...
        <View style={[styles.row, { gap: spacing.md }]}>
          <Field label={t('targetGpa')} value={target} onChangeText={setTarget} suffix={`/ ${SCALE_MAX}`} />
          <Field label={t('remainingCredits')} value={remaining} onChangeText={setRemaining} />
        </View>
